...
function format_DES_output( )
{
   ...
   if ( document.stuff.destype[0].checked )
   {
...
function des_encrypt( inData, Key, do_encrypt )
{
   ...
   for( i=1; i<=16; i++ )
   {
      ...
      if ( i==1 || i==2 || i==9 || i == 16 ){
         shift_CD_1( CD );
         printBitsTable(CD, "CD"+i+": Circular Left shift of CD"+(i-1)+" by "+1,"CD"+i);
      }
      else{
         shift_CD_2( CD );
...
function generate_hex_key()
{
   var generated_key = '0123456789abcdef'.split('').map(function(v,i,a){
      return i>15 ? null : a[Math.floor(Math.random()*16)] }).join('');  
   
   document.stuff.key.value=generated_key;
}
